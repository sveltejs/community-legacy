<script>
  ...
  const siteDomain = "https://svelte-community.netlify.app";

  const titlePostfix = "• Svelte Community";
  ...
  const metaData = {
    ...
    "/404": {
      title: `404 ${titlePostfix}`
    }
  };

  // remove trailing slash that is generated by netlify
  path = path.replace(/\/$/, '');

  if (!metaData[path]) path = "/404";
</script>
...
<svelte:head>
  ...
  <meta name="twitter:domain" value={siteDomain} />
